...
## Gotchas
- `install.sh` is a developer-local install script. It hardcodes a source path under `/Download/title/...` and writes to `/var/lib/jellyfin/...`. It is **not** a generic installer; the supported path is the Jellyfin plugin repository described in `README.md`. It takes an optional version argument (e.g. `./install.sh 1.1.3`); with no argument it reads the current csproj `<Version>` and auto-increments the 3rd component (e.g. `1.1.3.0` → `1.1.4`). In both cases it patches the csproj to `${version}.0`, runs `dotnet build -c Release`, then deploys the DLL and a generated `meta.json` whose `version` matches the argument. When using `release.sh` instead, the csproj `<Version>` and the manifest's `version` field must be bumped together.
